...
export class Tabs extends Ion {
    constructor(parent, viewCtrl, _app, config, elementRef, _plt, renderer, _linker, keyboard) {
        ...
        if (keyboard && keyboardResizes) {
            keyboard.willHide.pipe(takeUntil(this._onDestroy)).subscribe(() => {
                this._plt.timeout(() => this.setTabbarHidden(false), 50);
            });
            keyboard.willShow.pipe(takeUntil(this._onDestroy)).subscribe(() => this.setTabbarHidden(true));
        }
    }
    /**
     * @internal
    ...
    ngAfterViewInit() {
        ...
        if (this.tabsHighlight) {
            this._plt.resize.pipe(takeUntil(this._onDestroy)).subscribe(() => this._highlight.select(this.getSelected()));
        }
        this.initTabs();
    }
    /**
